...
def llm_stream(prompt_manager, log_and_call_manager, output_manager, chain_id: str, messages: str, model: str,
    ...
    try:
        ...
        # Fallback to tiktoken estimation if API didn't return usage
        if total_tokens_used == 0:
            encoding = tiktoken.encoding_for_model("gpt-4")
            full_content = ''.join([m for m in collected_messages])
            completion_tokens_used = len(encoding.encode(full_content))
        ...
        error_message = e.body.get('message') if hasattr(e, 'body') and e.body else str(e)
        output_manager.display_system_messages(f"MiniMax API Error: {error_message}")
        raise
